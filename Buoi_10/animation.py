...
def animationWidget(confirm):
    index = confirm
    if index < len(mario.list_run):
        changeImage(mario.run(), map.next_frame())
        index += 1
        root.after(80, lambda: animationWidget(index))
    return 0
...
button_1 = Button(root, text="Di chuyển", command=lambda: animationWidget(0))
button_2 = Button(root, text="Dừng lại", command=lambda: animationWidget(0))
